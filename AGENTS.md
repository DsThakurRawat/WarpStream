...
-   `test-interop`: Runs end-to-end interoperability tests between `warpstream` and the original Legacy `warpstream` implementation.
...
-   **Purpose:** To validate compatibility with the original Legacy `warpstream` implementation.
-   **Execution:** Run `make test-interop`.
-   **Requirements:** Requires the Legacy `warpstream` binary to be available on the system `PATH` under the name `warpstream` (tests will be skipped if it is not found).
